...
class ProductList extends React.Component {
  ...
  searchData = async () => {
    const resault = await request.get(`/products?filter=like(name,*${this.props.search}*)`)
    const data = resault.data.data.map(item => {
      let image = 'https://via.placeholder.com/300x400.png';
      if (item.relationships.main_image) {
        const fileId = item.relationships.main_image.data.id
        const file = resault.data.included.main_images.find(function(e) {
          return fileId === e.id;
        });
        image = file.link.href
      }
